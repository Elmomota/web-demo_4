...
export class AddKitPage implements OnInit {
  ...
  guardar() {
    ...
    this.kitService.crearKit(this.id_usuario, payload).subscribe({
      next: () => {
        this.presentAlert('Éxito', 'Kit agregado correctamente.');
        this.modalCtrl.dismiss({ recargar: true });
      },
      error: (err) => {
        console.error('Error al agregar kit:', err);
        this.presentAlert('Error', 'No se pudo agregar el kit.');
      }
    });
  }
